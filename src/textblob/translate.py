...
class Translator:
    ...
    def translate(self, source, from_lang="auto", to_lang="en", host=None, type_=None):
        ...
        data = {"q": source}
        url = "{url}&sl={from_lang}&tl={to_lang}&hl={to_lang}&tk={tk}&client={client}".format(  # noqa: E501
            url=self.url,
            from_lang=from_lang,
            to_lang=to_lang,
    ...
    def detect(self, source, host=None, type_=None):
        ...
        data = {"q": source}
        url = "{url}&sl=auto&tk={tk}&client={client}".format(
            url=self.url,
            tk=_calculate_tk(source),
            client="te",
        )
...
def _unescape(text):
    ...
    return re.sub(
        pattern, lambda x: codecs.getdecoder("unicode_escape")(x.group())[0], text
    )
